...
// Get Curs
  ...
  // Create query
      //print_r($this->conn);
        $test = "SELECT curs_id,titlu_curs,durata_curs,poza_prezentare,nivel,scurta_descriere,pret_reducere,pret FROM curs where status =:status";
// Prepare statement
        $connection = $this->conn;
        $stmt = $connection->prepare($test);
        $stmt->execute(array(":status"=>'activ'));
        return $stmt;
...
$query = 'SELECT C.titlu_curs as TITLU_CURS,C.sub_titlu AS SUB_TITLU, C.msg_pers_targetate AS MSG_PERS_TARGETATE, C.descriere AS DESCRIERE, C.nivel AS NIVEL, GR.data_start AS DATA_START,C.durata_curs AS DURATA_CURS,C.numar_ore_saptamana AS NUMAR_ORE_SAPTAMANA,
         ...
         FROM curs C
           INNER JOIN grupa_studenti GR ON GR.curs_id = C.curs_id WHERE C.curs_id=:CURS_ID AND GR.status =1';
// Prepare statement
...
// Create query
$query = "SELECT tehnologie from curs_tech where curs_id=:CURS_ID and type ='tehnologii_folosite' order by orderNumber";
// Prepare statement
    $connection = $this->conn;
$stmt = $connection->prepare($query);
$stmt->bindParam(':CURS_ID', $this->curs_id);
...
// Create query
$query = "SELECT tehnologie from curs_tech where curs_id=:CURS_ID and type ='cunostinte_de_baza' order by orderNumber";
// Prepare statement
    $connection = $this->conn;
$stmt = $connection->prepare($query);
$stmt->bindParam(':CURS_ID', $this->curs_id);
...
// Create query
$query = "SELECT tehnologie from curs_tech where curs_id=:CURS_ID and type ='mindset' order by orderNumber";
// Prepare statement
    $connection = $this->conn;
$stmt = $connection->prepare($query);
$stmt->bindParam(':CURS_ID', $this->curs_id);
